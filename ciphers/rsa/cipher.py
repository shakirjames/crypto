...
def encrypt(public_key_file, infile, outfile):
    logging.debug('cipher encrypt %s %s %s', public_key_file.name,
                  infile.name, outfile.name)
    key = public_key_file.read()
    message = infile.read()
    return encryptAndWriteToFile(key, message, outfile)


def decrypt(private_key_file, infile, outfile):
    logging.debug('decrypt encrypt %s %s %s', private_key_file.name,
                  infile.name, outfile.name)
    key = private_key_file.read()
    return readFromFileAndDecrypt(key, infile, outfile)
...
def parse_key(key):
    ...
    # content = fo.read()
    # fo.close()
    key_size, n, e_or_d = key.split(',')
    return (int(key_size), int(n), int(e_or_d))


#def encryptAndWriteToFile(messageFilename, key, message, blockSize=DEFAULT_BLOCK_SIZE):
def encryptAndWriteToFile(key, message, outfile, blockSize=DEFAULT_BLOCK_SIZE):
    # Using a key from a key file, encrypt the message and save it to a
    # file. Returns the encrypted message string.
    keySize, n, e = parse_key(key)
    ...
    # fo.close()
    outfile.write(encryptedContent)
    outfile.close()
...
def readFromFileAndDecrypt(key, infile, outfile):
    # Using a key from a key file, read an encrypted message from a file
    # and then decrypt it. Returns the decrypted message string.
    keySize, n, d = parse_key(key)
    ...
    decryptedContent = decryptMessage(encryptedBlocks, messageLength,
                                      (n, d), blockSize)
